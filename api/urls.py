...
from .views import BlogViewSet,TestView
from . import views
...
urlpatterns = [
    ...
    # # lấy post theo id
    # path('blogs/get/<int:id>/', Test_GetBlog.as_view({'get':'get_blog'}), name='get_blog'),
    # #phân trnag
    # #http://localhost:8000/blogs/page/1?page=3
#     # http://localhost:8000/blogs/page/1?page=1
#     path('blogs/page/<int:id>', BlogViewSets.as_view({'get':'page'}), name='page'),
    ...
    path('blogs/<int:id>/', BlogViewSet.as_view({'get': 'get_blog_id'}), name='get_blog_id'),
    path('blogs/<int:id_1>/publish/', BlogViewSet.as_view({'put': 'publish_post'}), name='publish_post'),
    path('blogs/<int:id>/unpublic_post/', BlogViewSet.as_view({'put': 'unpublic_post'}), name='unpublic_post'),
    path('blogs/sort_by_date/', BlogViewSet.as_view({'get': 'sort_blogs_by_date'}), name='sort_blogs_by_date'),
    path('blogs/create/', BlogViewSet.as_view({'post': 'create'}), name='create'),
    path('blogs/<int:blog_id>/update/', BlogViewSet.as_view({'put': 'update'}), name='update'),
    # path('blogs/<int:id>/delete/', BlogViewSet.as_view({'delete': 'destroy'}), name='delete_blog'),
    #http://localhost:8000/blogs/page/1?page=3
   #http://localhost:8000/blogs/page/1?page=1
    path('blogs/page/<int:id>', BlogViewSet.as_view({'get':'page'}), name='page'),
    ...
    path('blogs/delete_log/<int:id>', BlogViewSet.as_view({'get': 'get_deleted_log_by_post'})),
    path('blogs/update_log/<int:id>', BlogViewSet.as_view({'get': 'get_updated_log_by_post'})),
